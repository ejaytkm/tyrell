...
LEFT JOIN affiliates Affiliate
  ON (
    (Affiliate.type = 1
      AND Affiliate.id = (JobsTools.affiliate_id)
      AND (Affiliate.deleted) IS NULL)
    OR
    (Affiliate.type = 3
      AND Affiliate.id = (JobsCareerPaths.affiliate_id)
      AND (Affiliate.deleted) IS NULL)
    OR
    (Affiliate.type = 2
      AND Affiliate.id = (JobsRecQualifications.affiliate_id)
      AND (Affiliate.deleted) IS NULL)
    OR
    (Affiliate.type = 2
      AND Affiliate.id = (JobsReqQualifications.affiliate_id)
      AND (Affiliate.deleted) IS NULL)
  )
